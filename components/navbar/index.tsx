...
//   // This log will help you see exactly what is in your Redux store
//   console.log("Navbar Redux State:", { isLogged, userInfo });
...
//   // It consistently reads `profile_url` from the userInfo object in Redux.
//   // The Redux store itself should handle the fallback logic.
...
//               src="/images/Kuepass.svg"
//               alt="Kuepass"
//               width={120} // It's good practice to provide width/height for local images
//               height={40}
//               className={styles.kuepass}
//             />
//           </Link>
...
//                           imageProps={{
//                             onError: (e: React.SyntheticEvent<HTMLImageElement>) => {
//                               console.error("Avatar image failed to load from src:", avatarSrc);
//                               e.currentTarget.src = "/images/avatar.png"; // Set a final fallback
//                             },
//                           }}
//                         />
//                         <div className={styles.userNameContainer}>
...
//         position="right"
//         size="75%"
//         title={<Image src="/images/Kuepass.svg" alt="Kuepass" width={120} height={40} />}
//       >
//         <Stack gap="md">
//           <TextInput
//             placeholder="Search events..."
  ...
  useEffect(() => {
    const searchEvents = async () => {
      ...
      try {
        ...
        const formattedResults = Array.from(uniqueEventsMap.values()).map((event: any) => ({
          id: event.id,
          title: event.title,
          value: event.title,
          label: event.title,
      ...
      <header className={styles.navFlexEnhanced}>
        <div className={styles.navContainer}>
          ...
          <div className={styles.searchContainer}>
            <Autocomplete
              placeholder="Search events..."
              leftSection={isSearching ? <Loader size={18} /> : <IconSearch size={18} stroke={1.5} />}
              data={searchResults}
              value={searchValue}
              onChange={setSearchValue}
              onOptionSubmit={(itemValue) => {
                const selectedEvent = searchResults.find(event => event.value === itemValue);
                if (selectedEvent) {
