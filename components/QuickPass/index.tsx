...
export const QuickPage: React.FC<QuickPageProps> = ({
  id = "why-kuepass-section", // Default id, can be overridden by prop
  title = "Why Choose Kuepass",
  btnText = "Get Started",
  btnLink = "/eventSchedule/createEventForm",
}) => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        const [entry] = entries;
        // Ensure we only set to true once if that's the desired effect for one-time animation
        if (entry.isIntersecting) {
          setIsVisible(true);
          // Optionally unobserve if you want the animation to run only once
          // observer.unobserve(entry.target); 
        } else {
          // If you want the animation to reset when out of view (and re-animate on re-entry)
          // setIsVisible(false); 
        }
      },
      {
    ...
    const currentRef = contentRef.current; // Capture ref value
    if (currentRef) {
  ...
  }, []); // Empty dependency array: observe on mount, unobserve on unmount

  return (
    // Add the id attribute to the main container
    <Container fluid id={id} className={styles.quickPageContainer}> 
      <Stack className={styles.cardStack}>
        <Text component="h2" className={styles.cardText}>
          {title}
        </Text>
        ...
        <Flex
          ref={contentRef} // This ref is for the animation trigger point
          className={`${styles.cardFlex} ${isVisible ? styles.animate : ''}`}
          direction={{ base: "column", md: "row" }}
          align={{ base: "center", md: "flex-start" }}
          gap="xl"
