...
export function EventNetworking({ eventId }: { eventId: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/events/${eventId}/networking/attendees/`, { headers: t ? { Authorization: `Bearer ${t}` } : {} })
      .then(r => r.json()).then(d => setAttendees(Array.isArray(d) ? d : d.results || d.attendees || []));
  }, [eventId]);

  const filtered = attendees.filter(a => !search || a.name?.toLowerCase().includes(search.toLowerCase()) || a.company?.toLowerCase().includes(search.toLowerCase()));

  const action = async (endpoint: string, targetId: string, msg?: string) => {
    const t = getAuthToken();
    if (!t) return alert("Please log in");
    await fetch(`${API_BASE_URL}/networking/${endpoint}/`, { method: "POST", headers: { Authorization: `Bearer ${t}`, "Content-Type": "application/json" }, body: JSON.stringify({ event_id: eventId, target_attendee_id: targetId, message: msg }) });
    alert("Sent!"); setMessageTarget(null); setMessageText("");
  };

  return (
    <div style={{ marginBottom: 32 }}>
      <h3 style={{ fontSize: "1.2rem", fontWeight: 700, margin: "0 0 16px", color: "#1a1a2e", display: "flex", gap: 12, alignItems: "center" }}>
        Attendee Networking <span className={`${styles.badge} ${styles.badgePurple}`}>{attendees.length}</span>
      </h3>
      {attendees.length === 0 ? (
        <div style={{ textAlign: "center", padding: "32px 16px", background: "#f8fafc", borderRadius: 12, border: "1px solid #e5e7eb" }}>
          <FaUser size={28} color="#9ca3af" style={{ marginBottom: 8 }} />
          <p style={{ margin: "8px 0 0", color: "#6b7280", fontSize: "0.95rem" }}>No attendees available for networking yet. Check back once more people have joined!</p>
        </div>
      ) : (
        <>
          <input placeholder="Search by name or company..." value={search} onChange={e => setSearch(e.target.value)}
            style={{ width: "100%", padding: "10px 16px", borderRadius: 10, border: "1px solid #e5e7eb", marginBottom: 16, outline: "none" }} />
          <div className={styles.itemList}>
            {filtered.slice(0, 10).map(a => (
              <div key={a.id} className={styles.itemCard}>
                <div style={{ width: 44, height: 44, borderRadius: "50%", background: "linear-gradient(135deg, #a78bfa, #7c3aed)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "1.1rem" }}>
                  {a.name?.charAt(0) || "A"}
                </div>
                <div className={styles.itemCardContent}>
                  <p className={styles.itemCardTitle}>{a.name}</p>
                  {(a.title || a.company) && <p className={styles.itemCardDescription} style={{ margin: 0, color: "#6b7280" }}>{a.title} {a.company && `• ${a.company}`}</p>}
                  {messageTarget === a.id && (
                    <div style={{ marginTop: 12 }}>
                      <Textarea size="xs" placeholder="Write a message..." value={messageText} onChange={e => setMessageText(e.currentTarget.value)} rows={2} />
                      <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                        <button className={styles.smallBtn} style={{ background: "#7c3aed", color: "#fff", borderColor: "#7c3aed" }} onClick={() => action("message", a.id, messageText)}>Send</button>
                        <button className={styles.smallBtn} onClick={() => setMessageTarget(null)}>Cancel</button>
                      </div>
                    </div>
                  )}
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  {!a.is_connected && <button className={styles.smallBtn} onClick={() => action("connect", a.id)}>Connect</button>}
                  {messageTarget !== a.id && <button className={styles.actionBtn} onClick={() => { setMessageTarget(a.id); setMessageText(""); }} title="Message"><FaComments color="#7c3aed" /></button>}
                </div>
              </div>
            ))}
          </div>
