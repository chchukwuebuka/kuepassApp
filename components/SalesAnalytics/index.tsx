...
import { Accordion, Container, Title, Text, Loader, Center, Alert } from '@mantine/core';
...
const SalesAnalyticsPage: React.FC<SalesAnalyticsPageProps> = ({ eventId }) => {
  ...
  useEffect(() => {
    if (!eventId) {
      setIsLoading(false);
      setError("No event selected. Please go back to the overview and select an event.");
      return;
    }
    ...
    const fetchTicketsForEvent = async () => {
      ...
      setError(null);
      try {
        const response = await authenticatedRequest<TicketData[]>(`${API_BASE_URL}/tickets/?event=${eventId}`);
  ...
  if (isLoading) {
    return <Center style={{ padding: '2rem' }}><Loader /></Center>;
  }
  ...
  return (
    <Container size="lg" py="xl" className={styles.analyticsContainer}>
      <Title order={2} mb="xs">Sales Analytics</Title>
      <Text c="dimmed" mb="xl">
        View sales forecasts and performance for each of your event&apos;s tickets.
      </Text>
