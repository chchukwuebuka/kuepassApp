...
// import { Text, Group, Badge, ScrollArea, Center, Tooltip, ActionIcon, Loader, Alert, Skeleton } from "@mantine/core"
// import { Check, X, ChevronDown, AlertCircle, Users, RefreshCw } from "lucide-react"
...
// const UserTable: React.FC<UserTableProps> = ({ eventId, searchQuery = "", filter = "all" }) => {
...
//     setError(null)
//     try {
//       const response = await authenticatedRequest<Attendee[]>(`${API_BASE_URL}/attendees/?event=${eventId}`, "GET")
...
//     let score = 0
//     if (attendee.phone_number) score += 2 // High priority for phone_number
//     if (attendee.name && attendee.name.trim()) score += 1 // Name is required but check for non-empty
//     if (attendee.is_validated) score += 1 // Favor validated records
//     if (attendee.validated_at) score += 1 // Favor records with validation date
//     return score
//   }
...
//       const existing = unique.find((u) => u.email.toLowerCase() === attendee.email.toLowerCase())
...
//         ? unique.map((u) => (u.email.toLowerCase() === attendee.email.toLowerCase() ? attendee : u))
//         : unique
//     }, [] as Attendee[])
...
//   const displayedAttendees = showAll ? filteredAttendees : filteredAttendees.slice(0, 10)
...
//         <Alert icon={<AlertCircle size={16} />} title="Error Loading Attendees" color="red">
//           {error}
//         </Alert>
//         <Center mt="md">
...
//             <Badge size="lg" radius="xl" color="green" className={styles.attendeeCount}>
...
//             <button className={styles.tableBTN} onClick={() => setShowAll(!showAll)}>
...
//             {searchQuery ? "Try adjusting your search criteria." : "Attendees will appear here once they register."}
//           </Text>
//         </div>
//       ) : (
//         <ScrollArea>
...
//                       <Badge color="green" variant="filled" radius="xl" className={styles.validBadge}>
//                         <Check size={12} className={styles.badgeIcon} /> Validated
//                       </Badge>
//                     ) : (
//                       <Badge color="gray" variant="outline" radius="xl" className={styles.pendingBadge}>
...
//             View {filteredAttendees.length - 10} more attendees <ChevronDown size={16} className={styles.btnIcon} />
//           </button>
//         </div>
//       )}
//     </div>
...
const UserTable: React.FC<UserTableProps> = ({ eventId, searchQuery = "", filter = "all" }) => {
  ...
  const fetchAttendees = async () => {
    ...
    try {
      ...
      const response = await authenticatedRequest<Attendee[]>(`${API_BASE_URL}/attendees/?event_id=${eventId}`, "GET")
      console.log("UserTable attendees:", response) // Debug log
      setAttendees(response || [])
    } catch (err: any) {
      console.error("Fetch error:", err)
