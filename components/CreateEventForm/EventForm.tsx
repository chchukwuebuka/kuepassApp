...
const CreateEventForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (savedDraft) {
      try {
        ...
        if (parsedDraft.formData && typeof parsedDraft.currentStep === 'number') {
          console.log("CreateEventForm: Draft found, setting state.", parsedDraft);
          const loadedFormData = {
            ...initialFormData, // Start with defaults
            ...parsedDraft.formData,
            appearance: (parsedDraft.formData.appearance && parsedDraft.formData.appearance.startsWith("blob:")) 
          ...
          console.log("CreateEventForm: Saved draft was in unexpected format, using initial.");
          setFormData(initialFormData);
          setCurrentStep(1);
        }
      } catch (error) {
        console.error("CreateEventForm: Error parsing draft from localStorage:", error);
        setFormData(initialFormData);
        setCurrentStep(1);
      }
    } else {
      console.log("CreateEventForm: No draft found in localStorage, using initial.");
      setFormData(initialFormData);
      setCurrentStep(1);
    }
    setSelectedFile(null);
  ...
  useEffect(() => {
    ...
    console.log("CreateEventForm: Saving draft to localStorage. Step:", currentStep);
    const draftToSave = {
      formData: {
        ...formData,
  ...
  }, [formData, currentStep, isInitializing, selectedFile, initialFormData.appearance]);

  const createEvent = async (data: Partial<Omit<EventFormData, 'tickets' | 'questions' | 'appearance' | 'cardColor' | 'eventURL' | 'startDate' | 'startTime' | 'endDate' | 'endTime'> & {start_date: string; end_date: string}>) => {
    const res = await authenticatedRequest<{ data: {id: string} & EventFormData }>(
      `${API_BASE_URL}/events/`, "POST", data
    );
    if (!res || !res.data || !res.data.id) throw new Error("Event creation failed or ID not returned by API.");
    return res.data;
  };

  const createEventCustomization = async (data: { event: string; banner_url: string; font: string; card_color: string; is_active: boolean; }) => {
    console.log("Event Customization Payload for event ID:", data.event, JSON.stringify(data, null, 2));
    try {
  ...
  const createQuestion = async (data: { event_id: string; type: string; title: string; required: boolean; placeholder: string | null; order: number; }) => {
    await authenticatedRequest( `${API_BASE_URL}/questions/`, "POST", data );
  };

  const createTicketType = async (data: { event: string; category_name: "Paid" | "Free" | "Invite"; category_price: number; name: string; quantity?: number | string | null; }) => {
    const payload = { 
        event: data.event, 
        category_name: data.category_name, 
        category_price: data.category_price.toFixed(2), // Backend expects string for DecimalField
        name:data.name, 
        quantity: data.quantity === "Unlimited" ? "Unlimited" : data.quantity?.toString() 
    };
  ...
  const uploadImage = async (file: File): Promise<string> => {
    const formD = new FormData(); formD.append("image", file);
    console.log("uploadImage: Calling backend /upload-image/");
    const res = await authenticatedRequest<{success: boolean; message: string; data?: { url: string }; url?: string}>( // Allow for direct url or nested
    ...
    if (!res.success || !imageUrl) throw new Error((res as any).message || "Image upload failed or URL not returned.");
    if (!isValidUrl(imageUrl)) throw new Error(`Invalid image URL from upload: ${imageUrl}`);
    return imageUrl;
  };
  ...
  const handleLocationChange = (value: "Virtual" | "Physical") => { setFormData((p) => ({ ...p, location: value, address: value === "Virtual" ? "" : p.address })); };
  const handleUpdateQuestions = (questions: Question[]) => { setFormData((p) => ({ ...p, questions }));};
  ...
  const addTicket = (ticketDataFromModal: Omit<Ticket, "id">) => {
    ...
    if (isNaN(price) || (ticketDataFromModal.type === "Paid" && price < 0) || (ticketDataFromModal.type === "Free" && price !==0)) {
      alert("Please enter a valid ticket price."); return;
    }
    if (!ticketDataFromModal.name) { alert("Ticket name is required."); return; }
  ...
  const handleFileSelect = (file: File | null) => {
    setSelectedFile(file);
    if (file) {
      setFormData(prev => ({ ...prev, appearance: URL.createObjectURL(file) }));
    } else if (initialFormData) { 
      setFormData(prev => ({ ...prev, appearance: initialFormData.appearance }));
    }
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!formData.title) throw new Error("Event title is required.");
      if (formData.tickets.length === 0) throw new Error("At least one ticket type is required.");
      ...
      if (!isValidUrl(finalBannerUrl) || finalBannerUrl.startsWith("blob:")) {
          ...
          console.warn("handleSubmit: bannerUrl was invalid or blob after all checks, defaulting to placeholder.");
      }
      
      // Validate cardColor before sending
      if (!isValidHex(formData.cardColor)) { // This is where the error was
        throw new Error("Invalid card color format. Please use a 6-digit hex code (e.g., #RRGGBB).");
      }
      ...
      for (const q of formData.questions) {
        await createQuestion({
          event_id: eventId, type: q.type || "text", title: q.title,
          required: q.required || false, placeholder: q.placeholder || null,
          order: q.order || 0,
        });
      }
      ...
      for (const t of formData.tickets) {
        await createTicketType({
          event: eventId, category_name: t.type,
          category_price: t.type === "Free" ? 0 : Number(t.price),
          name: t.name || "General Ticket", quantity: t.quantity,
        });
      }
  ...
  const renderStepContent = () => { 
    switch (currentStep) {
      case 1: return ( <DetailsStep formData={formData} handleChange={handleChange} handleLocationChange={handleLocationChange} /> );
      case 2: return ( <TicketsStep tickets={formData.tickets} openModal={() => setIsModalOpen(true)} handleTicketTypeChange={handleTicketTypeChange} handleSendInvite={handleSendInvite} /> );
      case 3: return ( <CustomQuestionsStep questions={formData.questions} setQuestions={handleUpdateQuestions} /> );
      case 4: return ( <AppearanceStep formData={formData} updateFormData={(u) => setFormData((p) => ({ ...p, ...u }))} onFileSelect={handleFileSelect} /> );
      default: return null;
    }
  };

  if (isInitializing) {
    return <Center style={{height: '100vh'}}><Loader /> <Text ml="sm">Loading draft...</Text></Center>;
  }
  ...
  return (
    <div className={styles.formContainer}>
      ...
      <form onSubmit={handleSubmit} className={styles.eventForm}>
        {renderStepContent()}
        <NavigationButtons currentStep={currentStep} handleBack={handleBack} isLastStep={currentStep === 4} isLoading={isLoading} />
      </form>
      <TicketModal isModalOpen={isModalOpen} closeModal={() => setIsModalOpen(false)} addTicket={addTicket} />
    </div>
  );
};
