...
const getCurrentUser = async (): Promise<CurrentUser | null> => {
  ...
  try {
    ...
    if (!response.ok) {
        if (response.status === 401) localStorage.removeItem("kuepass_auth_token");
        return null;
    }
    const user = await response.json();
...
export default function EventDetails() {
  ...
  const handleGenerateCheckInQR = async () => {
    ...
    try {
      ...
      console.error("EventDetails - Check-In QR Code generation failed:", error);
      setCheckInQrError("Failed to generate QR code.");
    }
  };
  ...
  const handleGenerateEventPageQR = async () => {
    ...
    try {
      ...
      console.error("EventDetails - Event Page QR Code generation failed:", error);
      setEventPageQrError("Failed to generate QR code for the event page.");
    }
  };
    ...
    return (
      <Center style={{ height: "100vh", padding: "20px" }}>
        <Paper shadow="xs" p="xl" withBorder>
          <Stack align="center">
            <AlertTriangle size={48} color="red" />
            <Title order={3} ta="center">{pageError || "Event Not Found"}</Title>
            <Button component={Link} href="/events" leftSection={<ArrowLeft size={16} />}>
              Back to Events
            </Button>
          </Stack>
        </Paper>
  ...
  return (
    <div className={styles.container}>
        <MantineModal opened={showEmailPromptModal} onClose={() => { setShowEmailPromptModal(false); setShowCheckInQRModal(false); }} title="Enter Your Registration Email" centered >
            <Stack align="center" gap="md">
                ...
                <Button onClick={handleGuestEmailForCheckInQR}>Generate My Check-In QR</Button>
            </Stack>
        </MantineModal>
        <MantineModal opened={showCheckInQRModal && !showEmailPromptModal} onClose={() => setShowCheckInQRModal(false)} title="Your Event Check-in QR Code" centered size="auto">
            <Stack align="center" gap="md">
                {checkInQrError && <Text color="red">{checkInQrError}</Text>}
                {checkInQrImage && !checkInQrError && (
                    <>
                        <Text size="sm" ta="center">Present this QR code at the event for check-in.</Text>
                        <Image src={checkInQrImage} width={256} height={256} alt="Event Check-in QR Code" />
                    </>
                )}
                {!checkInQrImage && !checkInQrError && <Loader />}
                <Button onClick={() => setShowCheckInQRModal(false)} variant="light" mt="md">Close</Button>
            </Stack>
        </MantineModal>
        <MantineModal opened={showEventPageQRModal} onClose={() => setShowEventPageQRModal(false)} title="Share Event via QR Code" centered size="auto">
            <Stack align="center" gap="md">
                {eventPageQrError && <Text color="red">{eventPageQrError}</Text>}
                {eventPageQrImage && !eventPageQrError && (
                    <>
                        <Text size="sm" ta="center">Scan this QR code to view the event page.</Text>
                        <Paper p="md" withBorder>
                            <Image src={eventPageQrImage} width={256} height={256} alt="Event Page QR Code" />
                        </Paper>
                        <Group>
                            <Button onClick={handleDownloadQR} leftSection={<Share2 size={16} />} variant="filled">Download QR Code</Button>
                            <Button onClick={() => setShowEventPageQRModal(false)} variant="light">Close</Button>
                        </Group>
                    </>
                )}
                {!eventPageQrImage && !eventPageQrError && <Loader />}
            ...
            <div className={styles.heroOverlay}>
                <Container size="xl" className={styles.heroContainer}>
                    <div className={styles.heroContent} ref={headerContentRef}>
                        ...
                        <Group className={styles.heroActions}>
                            {!userIsRegisteredAndValidated ? (
                                <Button className={styles.registerButton} leftSection={<UserPlus size={18} />} size="lg" onClick={handleRegisterNowClick}>Register Now</Button>
                            ) : (
                                <Button className={styles.registerButton} leftSection={<QrCodeIcon size={18} />} size="lg" onClick={handleGenerateCheckInQR}>My Check-In QR</Button>
                            )}
                            <Button variant="outline" className={styles.shareButton} leftSection={<Share2 size={18} />} onClick={handleShareLink} size="lg">{copied ? "Link Copied!" : "Share Event"}</Button>
                            <Button variant="outline" className={styles.shareButton} leftSection={<QrCodeIcon size={18} />} onClick={handleGenerateEventPageQR} size="lg">Event Page QR</Button>
                        </Group>
                    </div>
                </Container>
            </div>
                ...
                <Container size="xl">
                    <Paper className={styles.countdownContainer}>
                        <Group justify="space-between" align="center" className={styles.countdownHeader}>
                            ...
                            <Badge size="lg" className={styles.countdownBadge}>Don&lsquo;t Miss It!</Badge>
                        </Group>
                        <CountdownTimer targetDate={countdownDate} />
                    </Paper>
                </Container>
        ...
        <Container size="xl" className={styles.mainContent}>
                ...
                <Grid.Col span={{ md: 4 }}>
                    <Stack gap="lg">
                        <Paper className={styles.contentCard}>
                            ...
                            <Divider className={styles.cardDivider} />
                            <div className={styles.cardBody}><Stack gap="md"><div className={styles.detailItem}><Text fw={600} className={styles.detailLabel}>Start Date:</Text><Text>{formatDateOnly(event.start_date)}</Text></div><div className={styles.detailItem}><Text fw={600} className={styles.detailLabel}>End Date:</Text><Text>{formatDateOnly(event.end_date)}</Text></div><div className={styles.detailItem}><Text fw={600} className={styles.detailLabel}>Time:</Text><Text>{formatTime(event.start_date)} - {formatTime(event.end_date)}</Text></div>{event.location && (<div className={styles.detailItem}><Text fw={600} className={styles.detailLabel}>Location:</Text><Text>{event.location}</Text></div>)}{event.address && event.location === "Physical" && (<div className={styles.detailItem}><Text fw={600} className={styles.detailLabel}>Address:</Text><Text>{event.address}</Text></div>)}</Stack></div>
                        </Paper>
                        <Paper className={styles.contentCard}>
                            <div className={styles.cardHeader}><Title order={3} className={styles.cardTitle}><UserPlus size={20} className={styles.cardIcon} />Check-In / Status</Title></div>
                            <Divider className={styles.cardDivider} />
                            <Stack gap="xs" p="md">
                                {userIsRegisteredAndValidated ? (
                                    <Button onClick={handleGenerateCheckInQR} fullWidth leftSection={<QrCodeIcon size={16} />}>My Check-In QR</Button>
                                ) : (
                                    <Button component={Link} href={`/eventSchedule/registerEvent?eventId=${id}`} fullWidth leftSection={<UserPlus size={16} />}>Register for Event</Button>
                                )}
            ...
            <Group className={styles.actionGroup}>
                <Link href="/eventSchedule/exploreEvent">
                    <Button className={styles.backButton} leftSection={<ArrowLeft size={18} />} size="lg">Back to Events</Button>
                </Link>
            </Group>
        </Container>
    </div>
...
//           setCountdownDate(
//             new Date(
//               activeCountdown ? activeActiveCountdown.target_date : event.start_date
//             )
//           );
//         } else {
//           setCountdownDate(new Date(event.start_date));
...
//     // Check if user is authenticated or has a guest email
//     if (!currentUser && !localStorage.getItem("kuepass_guest_email")) {
//       setShowEmailPromptModal(true);
//       return;
//     }
...
//             att.email.toLowerCase() === userEmailForQR.toLowerCase())) &&
//         att.event === id &&
//         att.is_validated === true
//     );

//     if (!specificAttendee) {
//       setCheckInQrError(
//         "You are not registered for this event or your registration is not validated. Please register or contact support."
//       );
//       return;
//     }

//     try {
//       // Generate a URL for the QR code instead of JSON
//       const qrData = `${API_BASE_URL}/checkin?attendeeId=${specificAttendee.id}&eventId=${id}&ts=${Date.now()}`;
//       const qrImageData = await QRCode.toDataURL(qrData, {
//         width: 256,
//         margin: 2,
//         errorCorrectionLevel: "H",
...
//         error
//       );
//       setCheckInQrError("Failed to generate QR code. Please try again.");
//     }
//   };
...
//       >
//         <Stack align="center" gap="md">
//           {checkInQrError && <Text c="red">{checkInQrError}</Text>}
//           {checkInQrImage && !checkInQrError && (
//             <>
//               <Text size="sm" ta="center">
...
//       >
//         <Stack align="center" gap="md">
//           {eventPageQrError && <Text c="red">{eventPageQrError}</Text>}
//           {eventPageQrImage && !eventPageQrError && (
//             <>
//               <Text size="sm" ta="center">
//                 Scan this QR code to view the event page.
