...
//   // Add a refresh trigger when the page loads
//   useEffect(() => {
//     // Trigger a refresh of attendee data when the page loads
//     // This ensures we get the latest data after registration
//     if (id && isAuthCheckComplete) {
//       console.log("=== DEBUG: Triggering attendee refresh on page load ===");
//       setRefreshTrigger(Date.now());

//       // Add a second refresh after a short delay to ensure we get the latest data
//       setTimeout(() => {
//         console.log("=== DEBUG: Triggering delayed attendee refresh ===");
//         setRefreshTrigger(Date.now());
//       }, 2000);
//     }
//   }, [id, isAuthCheckComplete]);
...
//         console.log("Processed Attendee Data:", attendeeDataArray);
...
export default function EventDetails() {
  ...
  const fetchAttendees = async () => {
    if (!id || !isAuthCheckComplete) return;
  ...
  useEffect(() => {
    ...
    // Handle refresh query param
    if (searchParams.get("refresh") === "true") {
      console.log(
        "=== DEBUG: Refresh query param detected, fetching attendees ==="
      );
      fetchAttendees();
      setTimeout(() => fetchAttendees(), 1000); // Retry after 1 second
      setTimeout(() => fetchAttendees(), 3000); // Retry after 3 seconds
      router.replace(`/eventSchedule/eventDetails/${id}`);
    }
  }, [id, isAuthCheckComplete, searchParams, router]);
  ...
  const userIsRegisteredAndValidated = checkUserRegistration(
    currentUser,
    attendees
  );
